...
| `check_release_gate.py` | Fails unless testing/security/deployment checklists are `passed` (SEO `passed`/`n/a`) and `release_gate_open: true` | `pre-push` hook (on `v*` tags), `kpi-snapshot.yml` |
